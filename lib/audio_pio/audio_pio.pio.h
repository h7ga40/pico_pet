...
static const uint16_t audio_pio_program_instructions[] = {
            //     .wrap_target
    0x80a0, //  0: pull   block
    0x2097, //  1: wait   1 gpio, 23
    0x80a0, //  2: pull   block
    0x2017, //  3: wait   0 gpio, 23
    0x2098, //  4: wait   1 gpio, 24
    0xe02f, //  5: set    x, 15
    0x2018, //  6: wait   0 gpio, 24
    0x6001, //  7: out    pins, 1
    0x2098, //  8: wait   1 gpio, 24
    0x0046, //  9: jmp    x--, 6
    0x2097, // 10: wait   1 gpio, 23
...
static const uint16_t read_pio_program_instructions[] = {
            //     .wrap_target
    0x2097, //  0: wait   1 gpio, 23
    0x2017, //  1: wait   0 gpio, 23
    0x2098, //  2: wait   1 gpio, 24
    0xe02f, //  3: set    x, 15
    0x2018, //  4: wait   0 gpio, 24
    0x2098, //  5: wait   1 gpio, 24
    0x4001, //  6: in     pins, 1
    0x0044, //  7: jmp    x--, 4
    0x2097, //  8: wait   1 gpio, 23
    0x2098, //  9: wait   1 gpio, 24
    0xe02f, // 10: set    x, 15
    0x2018, // 11: wait   0 gpio, 24
    0x2098, // 12: wait   1 gpio, 24
    0x4001, // 13: in     pins, 1
    0x004b, // 14: jmp    x--, 11
    0x0001, // 15: jmp    1
            //     .wrap
...
static inline void audio_pio_program_init(PIO pio, uint sm, uint offset, uint data_pin, uint lrckl_pin) {
    ...
    pio_sm_init(pio, sm, offset, &sm_config);
    uint pin_mask = (1u << data_pin) | (3u << lrckl_pin);
    uint pin_dirs = (1u << data_pin) | (0u << lrckl_pin);
...
static inline void read_pio_program_init(PIO pio, uint sm, uint offset, uint read_pin, uint lrckl_pin) {
    ...
    pio_sm_init(pio, sm, offset, &sm_config);
    uint pin_mask = (1u << read_pin) | (3u << lrckl_pin);
    uint pin_dirs = (0u << read_pin) | (0u << lrckl_pin);
    pio_sm_set_pindirs_with_mask(pio, sm, pin_dirs, pin_mask);
